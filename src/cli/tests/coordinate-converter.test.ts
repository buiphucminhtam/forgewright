...
describe("T10: Edge - Origin Point (0,0,0)", () => {
  it("should return same position for origin", () => {
    ...
    expect(
      vectorsAlmostEqual(convertPosition(origin, "unity", "godot"), origin),
    ).toBe(true);

    expect(
      vectorsAlmostEqual(convertPosition(origin, "godot", "unity"), origin),
    ).toBe(true);
...
describe("Transform Validation", () => {
  ...
  it("should warn on zero scale", () => {
    ...
    const result = validateTransform(transform, "unity");
    expect(result.warnings.some((w) => w.includes("zero or negative"))).toBe(
      true,
    );
  });
});
