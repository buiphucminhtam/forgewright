...
def main():
    ...
    parser.add_argument("--hops", type=int, default=2, help="Number of hops to traverse")
    ...
    for node, attr in mem.graph.nodes(data=True):
        ...
        if args.query.lower() in content:
            ...
            neighbors = list(mem.graph.successors(node)) + list(mem.graph.predecessors(node))
            for n in neighbors:
                if n != node:
                    results.append({"id": n, "attr": mem.graph.nodes[n], "relation": "neighbor"})
