...
| Model-call estimated budgets fail closed before invocation | `BudgetLedger` in `mcp/src/runtime/model-call-gateway.ts` | `mcp/src/runtime/model-call-gateway.test.ts` | Enforced whenever callers use the gateway; atomic reservation/actual-cost settlement remains a required provider-integration follow-up. |
