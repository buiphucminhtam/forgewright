...
| P1.3 | **Partial implemented locally:** offloaded MCP tool output returns a compact bounded result plus a sanitized reference; dedup is limited to the two read-only MCP tools | Performance / Builder | P1.2 | Mutation-aware cache epochs and measured 60% token reduction remain pending |
| P1.4 | **Partial implemented locally in the model gateway:** estimated task/account preflight warns at 80%, returns authority-required at 95%, and fails closed at 100% unless construction-time overage policy is explicit | FinOps + runtime / Expert | P1.1 | Deterministic threshold/override evidence exists; atomic reservation/settlement and operational overshoot evidence require live-provider rollout work |
