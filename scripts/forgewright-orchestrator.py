...
class ForgewrightAgent:
    ...
    async def run(self, task: str):
        ...
        if not os.path.exists(manifest_path) and not os.path.exists(
            ...
            mcp_setup_script = os.path.join(_REPO_ROOT, "scripts", "forgewright-mcp-setup.sh")
            try:
                subprocess.run(
        ...
        if os.path.exists(
            ...
            mcp_launcher = os.path.join(_REPO_ROOT, "scripts", "forgewright-mcp-launcher.sh")
            mcp_servers.append(
                {
                    "name": "forgewright",
                    "params": StdioServerParameters(
...
if __name__ == "__main__":
    ...
    cdir = sys.argv[3] if len(sys.argv) > 3 else os.path.join(_REPO_ROOT, "projects", pid, "code")
