...
def load_completed_sessions(path: Path, limit: int) -> list[dict[str, Any]]:
    ...
    if not isinstance(sessions, list):
        return []
    completed = [s for s in sessions if isinstance(s, dict) and s.get("status") == "completed"]
...
def source_refs(
    ...
    filtered_events = [
        event for event in events if not session_id or str(event.get("session_id")) == session_id
    ]
...
def preference_bullets(observations: list[dict[str, Any]], max_items: int = 8) -> list[str]:
    bullets: list[str] = []
    for obs in observations:
        text = f"{obs.get('title', '')} {obs.get('content', '')}"
        if not PREFERENCE_PATTERNS.search(text):
            continue
        bullets.append(f"- {one_line(obs.get('content') or obs.get('title'), 220)} [mem0:{obs.get('id')}]")
        if len(bullets) >= max_items:
            break
    return bullets


def render_persona(observations: list[dict[str, Any]], events: list[dict[str, Any]]) -> str:
...
def render_scenario(
    ...
    if not tasks:
        tasks = [one_line(obs.get("title") or obs.get("content"), 180) for obs in observations[:5]]
    if not tasks:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Consolidate memory into persona/scenario layers")
...
def main() -> int:
    ...
    result = write_outputs(memory_bank, observations, sessions, events, args.max_scenarios)
