...
def evaluate_condition(condition: str, context: str = "") -> tuple[GoalResult, str]:
    ...
    if "test" in condition_lower and ("pass" in condition_lower or "passing" in condition_lower):
        return evaluate_tests(condition, context)

    # "lint is clean" / "lint passes"
    if "lint" in condition_lower and ("clean" in condition_lower or "pass" in condition_lower):
        return evaluate_lint(condition, context)

    # === FILE-BASED CONDITIONS ===

    # "N files" / "file count"
    file_match = re.search(r'(\d+)\s+(?:new\s+)?(?:file|module|service)s?', condition_lower)
    if file_match:
    ...
    if any(kw in condition_lower for kw in ["compiles", "builds", "build succeeds", "build pass"]):
    ...
    return "unknown", "Cannot auto-evaluate this condition type. Please verify manually."
...
def evaluate_tests(condition: str, context: str) -> tuple[GoalResult, str]:
    ...
    pass_indicators = ["passed", "ok", "pass", "✓", "success", "0 failed"]
    fail_indicators = ["failed", "error", "fail", "✗", "1 failed", "2 failed"]
...
def evaluate_lint(condition: str, context: str) -> tuple[GoalResult, str]:
    ...
    if not lint_cmd and os.path.exists(".eslintrc.js") or os.path.exists(".eslintrc.json"):
...
def evaluate_file_count(condition: str, context: str, expected: int) -> tuple[GoalResult, str]:
...
def evaluate_documentation(condition: str, context: str) -> tuple[GoalResult, str]:
    ...
    exit_code, stdout, _ = run_command("grep -r 'TODO.*doc' --include='*.py' --include='*.js' --include='*.ts' . 2>/dev/null | head -5")
