...
# Usage: bash scripts/verify-wiki-drift.sh [--threshold 0.3] [--verbose]
...
echo -e "\n${BOLD}[1/2] Đang quét mâu thuẫn chéo giữa các tài liệu (Doc-to-Doc)...${NC}"
...
echo -e "\n${BOLD}[2/2] Đang kiểm tra độ tươi mới của đồ thị GitNexus (Doc-to-Code)...${NC}"
...
if [ -d ".gitnexus" ]; then
    ...
    if command -v npx &>/dev/null; then
        if npx gitnexus status &>/dev/null; then
            ...
            echo -e "  ${YELLOW}⚠ Chỉ mục GitNexus có thể bị lệch hoặc chưa hoàn thiện.${NC}"
