...
export function createPiWorkerAdapter(config = {}) {
  ...
  function enforceDeadline() {
    // Timer callbacks may be starved by synchronous SDK/provider work. Monotonic
    // elapsed time also gates dispatch, message admission and successful exit.
    if (running && !interruption && performance.now() - startedAt >= limits.timeoutMs) stop('pi_timeout');
    ...
    async start(input) {
      ...
      const execution = (async () => {
        ...
        unsubscribe = agent.subscribe((event) => {
          ...
          if (performance.now() - startedAt >= limits.timeoutMs) { stop('pi_timeout'); return; }
