...
async function loadLanguage(lang: string): Promise<any> {
  switch (lang) {
    ...
    case 'python': {
      try {
        return (_require('tree-sitter-python') as any).default ?? null
      } catch {
        return null
      }
    }
...
function nodeText(node: any, content: string): string {
  ...
  return content.substring(
    posToIndex(content, s.row, s.column),
    posToIndex(content, e.row, e.column),
  )
}
...
function callTarget(node: any, content: string): string | null {
  ...
  if (node.type === 'member_expression') {
    const obj = node.children?.find(
      (c: any) => !['.', 'property_identifier', '*', '::'].includes(c.type),
    )
      ...
      for (const hc of childNode.namedChildren) {
        if (hc.type === 'extends_clause') {
          ...
          if (superClass) {
            ...
            edges.push({
              id: `${parentUid}->EXTENDS:${name}`,
              fromUid: parentUid,
              toUid: `${filePath}:Class:${name}:0`,
              type: 'EXTENDS',
              confidence: 1.0,
        ...
        if (hc.type === 'implements_clause') {
          for (const iface of hc.namedChildren) {
            ...
            if (name)
              edges.push({
                id: `${parentUid}->IMPLEMENTS:${name}`,
                fromUid: parentUid,
                toUid: `${filePath}:Interface:${name}:0`,
                type: 'IMPLEMENTS',
                confidence: 1.0,
...
function extractMembers(
  ...
  for (const member of body.namedChildren) {
    ...
    if (
      ...
      mt === 'function_definition' ||
      mt === 'method_declaration'
    ) {
    ...
    if (
      mt === 'property_declaration' ||
      mt === 'field_declaration' ||
      ...
      if (pName) {
        ...
        edges.push({
          id: `${parentUid}->HAS_PROPERTY:${pName}`,
          fromUid: parentUid,
          toUid: pUid,
          type: 'HAS_PROPERTY',
          confidence: 1.0,
