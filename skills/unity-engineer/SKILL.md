...
*   **Wait-for-Compile Mandate (Race Condition Prevention):** When writing a C# script to disk (`script_create` or file edit), the Unity Editor will recompile. You MUST call `wait_for_unity(timeout=10)` right after creating the file and before trying to attach that component. Never blindly spam REST requests!
*   **Bounding Box Spatial Awareness:** You are blind to 3D dimensions. Do NOT hardcode (0,0,0) or guess placements. You MUST query the bounds of a Renderer or Collider to calculate exact placement mathematics before moving objects.
