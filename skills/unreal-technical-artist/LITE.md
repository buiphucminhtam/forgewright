...
|---|---|---|---|
| Target Unreal project configuration exists | `find . -maxdepth 2 -name "*.uproject"` | ... | Y/N |
| Project-specific tech stack and profile settings are active | `cat .forgewright/project-profile.json` | ... | Y/N |
| Shader files, HLSL materials, or Niagara assets are indexed | `find Content/ -name "*.uasset" -o -name "*.hlsl" -o -name "*.ush"` | ... | Y/N |
...
4. SYNC | Save technical specs as lowercase kebab-case under docs/ and run sync hooks | Trigger standard post-skill sync scripts to establish absolute symlinks for documentation.
