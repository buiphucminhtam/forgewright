...
| Active linters and code formatting packages are configured | `cat package.json \| jq '.devDependencies \| keys' \| grep -E \"(eslint\|prettier\|biome)\"` | ... | Y/N |
| GitNexus symbol index and call graphs are present and initialized | `gitnexus analyze --status \|\| find . -name \"*.gitnexus\"` | ... | Y/N |
