...
|---|---|---|---|
| Active session logs, raw history files, or offload traces exist | `find .forgewright/offload/ -name \"*.md\" \|\| find ~/.forgewright/usage/ -name \"*.jsonl\"` [1, 2] | ... | Y/N |
| Memory hygiene, garbage collection, or compression scripts exist | `find scripts/ -name \"*memory-hygiene*\" -o -name \"*compress*\"` | ... | Y/N |
| Twin-middleware execution properties and context offload thresholds are defined | `cat .production-grade.yaml` [1, 4] | ... | Y/N |
...
2. COMPACT | Execute memory-hygiene scripts to compress session logs and prune duplicate token records [3, 7] | Verify that SHA-256 deduplication runs clean and successfully merges redundant query traces to save up to 90% space.
