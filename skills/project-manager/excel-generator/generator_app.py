...
def calc_proposed_effort(role, task):
    ...
    if any(k in t for k in ["ui", "shop", "screen", "design"]):
        if "ui/ux" in r or "2d" in r:
            return 0.85
        if "developer" in r:
            return 0.4
        return 0.2
    if any(k in t for k in ["3d", "model", "boss", "map", "animation"]):
        if "3d" in r or "animator" in r:
            return 0.95
        if "concept" in r:
            return 0.7
        return 0.2
    if "developer" in r or "artist" in r:
        return 0.45
    if "qa" in r or "qc" in r:
        return 0.4
    if "manager" in r:
        return 0.2
    return 0.1
...
def generate_project_files(config_path):
    ...
    plan_out = os.path.join(
        base_dir, f"{config['project_name'].replace(' ', '_')}_Plan.xlsx"
    )
    cost_out = os.path.join(
        base_dir, f"{config['project_name'].replace(' ', '_')}_Cost.xlsx"
    )

    print(" -> Instantiating Templates...")
    shutil.copy(
        os.path.join(base_dir, "templates", "Project_Plan_Template.xlsx"), plan_out
    )
    shutil.copy(
        os.path.join(base_dir, "templates", "Cost_Planning_Template.xlsx"), cost_out
    )
        ...
        for sp in range(ms["sprints_count"]):
            for epic in ms["backlog_epics"]:
                for member in config["team"]:
                    role = member["role"]
                    role_efforts[role] = role_efforts.get(
                        role, 0
                    ) + calc_proposed_effort(role, epic)

    role_capacity = {}
    for member in config["team"]:
        role = member["role"]
        role_capacity[role] = role_capacity.get(role, 0) + float(
            config["project_duration_months"]
        )
    ...
    for role, tot in role_efforts.items():
        ...
        if tot > cap:
            tbd_insertions[role] = math.ceil(
                (tot - cap) / float(config["project_duration_months"])
            )
    ...
    for t in config["team"]:
        ...
        res.cell(
            row=base_row, column=9
        ).value = f"=E{base_row}*{config['project_duration_months']}"  # EFFORT THỰC TẾ
        ...
        if t["role"] in tbd_insertions and t["role"] not in tbd_spawned:
            ...
            for t_idx in range(tbd_count):
                ...
                for c in range(1, 10):
                    res.cell(row=t_row, column=c).font = copy.copy(
                        res.cell(row=base_row, column=c).font
                    )
                    res.cell(row=t_row, column=c).alignment = copy.copy(
                        res.cell(row=base_row, column=c).alignment
                    )
                    res.cell(row=t_row, column=c).border = copy.copy(
                        res.cell(row=base_row, column=c).border
                    )
                    res.cell(row=t_row, column=c).number_format = res.cell(
                        row=base_row, column=c
                    ).number_format
        ...
        for p_idx in range(base_row, base_row + tbd_count + 1):
            ...
            consumed = (
                f'SUMIFS(H$56:H{p_idx - 1}, C$56:C{p_idx - 1}, "*{clean_role}*")'
                if p_idx > 57
                else "0"
            )
            res.cell(
                row=p_idx, column=8
            ).value = f"=MIN({config['project_duration_months']}, MAX(0, {sum_logic} - {consumed}))"
            res.cell(
                row=p_idx, column=4
            ).value = f"=H{p_idx}/{config['project_duration_months']}"
        ...
        for sp_idx in range(ms["sprints_count"]):
            ...
            for epic in ms["backlog_epics"]:
                for member in config["team"]:
                    ...
                    ds.cell(
                        row=cur_row, column=2
                    ).value = f"[{epic}] - Review & Implement"
                    ds.cell(row=cur_row, column=3).value = member["role"]
                    ds.cell(row=cur_row, column=6).value = calc_proposed_effort(
                        member["role"], epic
                    )
            ...
            if task_rows:
                ds.cell(
                    row=sp_row, column=6
                ).value = f"=SUM({','.join([f'F{r}' for r in task_rows])})"
            sprint_rows.append(sp_row)
        if sprint_rows:
            ds.cell(
                row=ms_row, column=6
            ).value = f"=SUM({','.join([f'F{r}' for r in sprint_rows])})"
