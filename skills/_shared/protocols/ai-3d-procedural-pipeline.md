...
- **85% (Môi trường, Đồ vật, Thực vật):** Tuyệt đối cấm dùng thuật toán nặn lưới (Mesh Generation) bằng Code hay dùng khối Primitives (Cube/Sphere) của Unity để làm hình ảnh cuối cùng (chỉ được dùng lúc Greybox blockout). Asset **PHẢI ĐƯỢC LẮP GHÉP (Kitbash)** từ thư mục `Assets/Resources/Kitbash/` (do User tải sẵn từ Kenney.nl, AssetStore...). AI có trách nhiệm quét thư mục này, lấy UUID và rải tọa độ `(x, y, z)` ra bản đồ.
