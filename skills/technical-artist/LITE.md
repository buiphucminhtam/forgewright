...
|---|---|---|---|
| Target assets and metadata schemas are in correct project folders | `find assets/ -name "*.meta" -o -name "*.json"` | Lists available metadata or asset configurations [1] | |
| Active shader compiler or engine pipelines are available (e.g., glslangValidator, Node/Three) | `which glslangValidator || npm list -g` | Identifies available CLI asset or shader compiling tools [2] | |
| Project-specific tech stack and render pipeline are onboarded | `cat .forgewright/project-profile.json` | Displays target engine (e.g., Unity, Three.js, Godot) [3, 4] | |
| Spending budget and token limits are configured for batch operations | `cat .forgewright/budget.yaml` | Displays configured API token thresholds and limits [5, 6] | |
...
2. OPTIMIZE | Compile/minify shaders or strip unused mesh metadata and vertex attributes | Confirm vertex counts and draw calls are minimized to avoid performance bottlenecks [5].
...
- **Leaked material instances**: Instantiating materials programmatically in updates without proper memory tracking, leading to continuous WebGL or engine GPU leaks [8].
- **Ignoring asset naming conventions**: Storing meshes or material files with uppercase letters or space separators instead of strictly lowercase kebab-case in asset directories [9].
...
```json
{
  "project_name": "forgewright-xr-game",
...
Output:
```
[SUCCESS] assets/shaders/custom-pbr.frag compiled successfully with 0 warnings.
[PERF] Vertex/Fragment shader within optimal register count budget.
```
