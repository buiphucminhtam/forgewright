...
|---|---|---|---|
| Target IDE configuration (Cursor, Claude Desktop) is readable and writable | `cat ~/.cursor/mcp.json \| jq '.mcpServers["unity-mcp"]'` | ... | Y/N |
| Project-specific Antigravity manifest or assembly definitions exist | `find . -name "*.asmdef" -o -name "mcp-manifest.json"` | ... | Y/N |
| Local Unity socket bridge endpoints are active or listening | `ss -tlnp \| grep -E "(5000\|3000)"` | ... | Y/N |
